...
- Incoming channel revenue threshold: the total routing fees over the maximum
  ...
  local node, considering both incoming and outgoing HTLCs that have used the
  channel.
...
to the local node), only outgoing HTLCs are considered.
...
- for each `update_add_htlc` offered or received on the incoming channel over
  the rolling window [`now` - `revenue_window`; `now`]:
  - if the HTLC has been resolved: 
    - `incoming_revenue_threshold` += `fee`

##### Rationale

We consider bi-directional HTLC forwards on the incoming channel to properly
...
We define the following:
- `general_bucket_slot_count` = 5
- `general_bucket_liquidity_allocation` =
  `general bucket capacity * general_bucket_slot_count / general bucket slot total`

Each `(incoming scid, outgoing scid)` is deterministically assigned slots:
```
for i = 0; i < general_bucket_slot_count {
...
It is highly improbable that two channels are granted access to exactly the
same set of resources, which makes it difficult for an attacker to entirely
crowd out honest traffic.
...
- MUST set `accountable` in the offered `update_add_htlc` as outlined in
  [HTLC Accountability](#htlc-accountability).
